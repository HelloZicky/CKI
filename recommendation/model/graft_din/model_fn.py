...
@model("graft_din", MetaType.ModelBuilder)
class DeepInterestNetwork(nn.Module):
    def __init__(self, model_conf):
        ...
        # debug
        # self._classifier_debug = common.StackedDense(
        #     model_conf.id_dimension * 2,
        #     model_conf.classifier + [1],
        #     ([torch.nn.Tanh] * len(model_conf.classifier)) + [None]
        # )

        self._classifier = common.Graft_StackedDense(
            model_conf.id_dimension * 2,
            model_conf.classifier + [1],
            ([torch.nn.Tanh] * len(model_conf.classifier)) + [None],
            model_name="graft_din",
        )

    def __setitem__(self, k, v):
        self.k = v

    def forward(self, features, graft_param, finetune=False):
        # Encode target item
        # B * D
        ...
        return self._classifier(classifier_input, graft_param, finetune)

    def load_classifier(self, graft_param):
        self._classifier.load_param(graft_param=graft_param)


# v1 2
class DeepInterestNetwork_v1(nn.Module):
    def __init__(self, model_conf):
        super(DeepInterestNetwork, self).__init__()
