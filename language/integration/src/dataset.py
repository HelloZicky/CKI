...
class OurInputFeatures(InputFeatures):
    ...
    query_type_ids: Optional[List[int]] = None
...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False):
        ...
        self.template_list = self.args.template_list
        ...
        #randomly permute  query examples
        #order = np.random.permutation( len(self.query_examples) )
        #print ( self.support_examples, len(self.support_examples) )
        #print ( self.query_examples, len(self.query_examples) )
        #self.support_examples = [ self.support_examples[order[i]] for i in range( len(order) ) ]
        #self.query_examples   = [ self.query_examples[order[i]] for i in range( len(order) ) ]
    ...
    def __getitem__(self, i):
        if self.features is None:
            ...
            if self.template_list is not None:
                template = self.template_list[sample_idx % len(self.template_list)]
            else:
